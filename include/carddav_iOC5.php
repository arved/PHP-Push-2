...
class carddav_backend
{
	 ...
	 * @constant	string
	 */
	const USERAGENT = 'CardDAV PHP OC/';
	...
	*/
	...
	public function get($include_vcards = true, $raw = false)
	{
		...
		//ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get:: http_code : '%s'", $result['http_code']));
		//ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get:: response : '%s'", $result['response']));
		switch ($result['http_code']){
			case 200:
			case 207:
				if ($raw === true){
					//ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get:: raw : '%s'", $result['response']));
					return $result['response'];
				}
				else{
					//ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get:: not raw (include, data) : '%s'", $include_vcards));
	...
	{
	 //ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get_all_vcards"));
	 $result = ($this->clean_response($this->get($include_vcards, $raw)));
	 //ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::get_all_vcards '%s'", print_r($result,true)));
        return $result;
        }   
	...
	*/
	...
	{	
		ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::add:: '%s', '%s'", $vcard_id , $vcard ));
		if (($vcard_id === null) || ($vcard_id == ''))
		{
		...
		ZLog::Write(LOGLEVEL_DEBUG, sprintf("IncludeCardDav::carddav_OC5.php::add:: '%s', '%s'", (string)$result['http_code'], $result['response']));
	...
	private function clean_response($response)
	{
		...
		$response = str_replace('c:', null, $response);
		// really ugly solution - any better idea?
		$response = str_replace(' xmlns:d="DAV:" xmlns:s="http://sabredav.org/ns" xmlns:card="urn:ietf:params:xml:ns:carddav"', null,$response);
		return $response;
	}
	...
	private function generate_vcard_id()
	{
		...
		$vcard_id = strtoupper(gmdate("Ymd\THis\Z") .'-'. substr($md5, 0, 4 ) . '-' . substr($md5, 4, 8));
